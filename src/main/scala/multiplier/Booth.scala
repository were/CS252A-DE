...
class Booth(width: Int) extends Module {
    ...
    when(io.a === 0.U || io.a === 7.U) {
        io.out := Cat(1.U(1.W), 0.U((width + 1).W))
        io.cout := 0.U
    } .elsewhen(io.a === 1.U || io.a === 2.U) {
        io.out := Cat(1.U(1.W), 0.U(1.W), io.b)
        io.cout := 0.U
    } .elsewhen(io.a === 5.U || io.a === 6.U) {
        io.out := Cat(0.U(1.W), 1.U(1.W), ~io.b)
        io.cout := 1.U
    } .elsewhen(io.a === 3.U) {
        io.out := Cat(1.U(1.W), io.b, 0.U(1.W))
        io.cout := 0.U
    } .otherwise {
        io.out := Cat(0.U(1.W), ~io.b, 1.U(1.W))
        io.cout := 1.U
    }
